...
import { Search, Calculator, Trash2, Building, DollarSign, Wrench, TrendingUp, CreditCard, Loader2, Home, AlertCircle } from 'lucide-react';
...
const PropertyForm = ({ 
  ...
  const getPropertyImage = () => {
    if (propertyData?.imageLink) return propertyData.imageLink;
    if (propertyData?.streetView) return propertyData.streetView;
    return null;
  };
      ...
      <motion.div className="form-section address-section" variants={itemVariants}>
        ...
        {fetchError && !propertyData && (
          ...
          >
            ...
            <span>{fetchError}. You can still enter values manually.</span>
          </motion.div>
        )}
        ...
        {propertyData && (
          ...
          >
            <div className="preview-header">
              ...
              {propertyData.homeType && propertyData.homeType !== 'Not available' && (
                <span className="preview-type-badge">{propertyData.homeType}</span>
              )}
            </div>
                ...
                <div className="property-image-container">
                  ...
                  <div className="image-overlay">
                    <span className="image-source">
                      {propertyData.imageLink ? 'Property Photo' : 'Street View'}
                    </span>
                  </div>
                </div>
              ) : (
            ...
            <div className="preview-grid">
              {propertyData.zestimate > 0 && (
                <div className="preview-item highlight">
                  ...
                  <span className="preview-value large">${propertyData.zestimate.toLocaleString()}</span>
                </div>
              )}
              {(propertyData.rentZestimate > 0 || propertyData.rent_zestimate > 0) && (
                <div className="preview-item highlight">
                  <span className="preview-label">Rent Estimate</span>
                  <span className="preview-value large">${(propertyData.rentZestimate || propertyData.rent_zestimate).toLocaleString()}/mo</span>
                </div>
              )}
              {propertyData.beds && propertyData.beds !== 'N/A' && (
                <div className="preview-item">
                  <span className="preview-label">Beds</span>
                  <span className="preview-value">{propertyData.beds}</span>
                </div>
              )}
              {propertyData.baths && propertyData.baths !== 'N/A' && (
                <div className="preview-item">
                  <span className="preview-label">Baths</span>
                  <span className="preview-value">{propertyData.baths}</span>
                </div>
              )}
              {(propertyData.livingArea > 0 || propertyData.living_area > 0) && (
                <div className="preview-item">
                  <span className="preview-label">Living Area</span>
                  <span className="preview-value">{(propertyData.livingArea || propertyData.living_area).toLocaleString()} sqft</span>
                </div>
              )}
              {propertyData.lotArea > 0 && (
                <div className="preview-item">
                  <span className="preview-label">Lot Size</span>
                  <span className="preview-value">{propertyData.lotArea.toLocaleString()} {propertyData.lotUnits || 'sqft'}</span>
                </div>
              )}
              {(propertyData.yearBuilt > 0 || propertyData.year_built > 0) && (
                <div className="preview-item">
                  <span className="preview-label">Year Built</span>
                  <span className="preview-value">{propertyData.yearBuilt || propertyData.year_built}</span>
                </div>
              )}
              {propertyData.hoaFee > 0 && (
                <div className="preview-item">
                  <span className="preview-label">HOA Fee</span>
                  <span className="preview-value">${propertyData.hoaFee.toLocaleString()}/mo</span>
                </div>
              )}
              {propertyData.heating && propertyData.heating !== 'Not available' && (
                <div className="preview-item">
                  <span className="preview-label">Heating</span>
                  <span className="preview-value">{propertyData.heating}</span>
                </div>
              )}
            </div>
          </motion.div>
